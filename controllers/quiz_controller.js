  ...
  models.Favourites.findAll({ where: { 
	UserId: Number(req.session.user.id) }}
		...
		var h; 
 			...
 			esFav[j] = "noFav";
                     for (h=0; h<favoritos.length;h++) {
 			if (favoritos[h].QuizId === quizes[j].id) {
					esFav[j] = "esFav";
 			}
 	  	     }
		}
 	   res.render('quizes/index.ejs', { quizes: quizes, esFav: esFav, errors: []});
         });
 }).catch(function(error){next(error)});
 } else {
...
exports.create = function(req, res) {
 ...
 function(err){
  ...
  quiz // save: guarda en DB campos pregunta y respuesta de quiz
   .save({fields: ["pregunta", "respuesta", "categoria", "UserId", "image"]})
...
exports.busqueda = function(req, res){
   models.Quiz.findAll().then(function(quizes) {
    res.render('quizes/busqueda', {quizes: quizes, buscar: req.query.busqueda, errors: []});
   })
};
